...
n_embd = 384 #32 # also being used a head_size
n_head = 6
n_layers = 12
dropout = 0.2
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        wei = q @ k.transpose(-2,-1) * self.head_size**-0.5 # (B, T, self.head_size) @ (B, self.head_size, T ) --> (B, T, T)
        ...
        out = wei @ v # (B, T, T) @ (B, T, self.head_size) --> (B, T, self.head_size)
        return out
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        #                             nn.LayerNorm(n_embd),
        #                             )
        self.blocks = nn.Sequential(*[Block(n_embd=n_embd, n_head=n_head) for _ in range(n_layers)])
...
model.to(device)
print(f'Number of parameters in the model is: {sum(p.numel() for p in model.parameters())}')

optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate)

for iter in range(max_iters):
    # every once in while evaluate the loss of train & val sets
